...
For the sample output type, the LOP method pools the input sample predictions into a single ensemble distribution. By default, the `linear_pool()` function will simply collect and return all provided samples. However, the user may also specify a number of sample predictions for the ensemble to return using the `n_output_samples` argument, in which case a stratified draw will be performed to select a subset of predictions to create the linear pool of samples so that all component models are represented equally.
...
linear_pool_norm <- hubExamples::forecast_outputs |>
  dplyr::filter(!output_type %in% "median") |>
...
## Changing distributional tails estimate for quantiles

We can also change the distribution that `distfromq` uses to estimate the component models' distributional tails for quantiles to either log normal or Cauchy using the `tail_dist` argument. This choice usually does not have a large impact on the resulting ensemble distribution, though, and can only be seen in its outer edges. (For more details and other function options, see the documentation in the [`distfromq` package](https://reichlab.io/distfromq/).)

```{r}
linear_pool_lnorm <- hubExamples::forecast_outputs |>
  dplyr::filter(output_type %in% "quantile") |>
  hubEnsembles::linear_pool(
...
The sample output type is unique in that it may represent predictions from either joint or marginal predictive distributions, unlike the other output types which only ever summarize the latter. Hence, sample representation may capture dependence across combinations of multiple modeled task ID variables when recording samples from a joint predictive distribution. In this case, samples with the same index (specified by the `output_type_id`) may be assumed to correspond to a single sample from a joint distribution across multiple levels of the task id variables.

When requesting a linear pool made up of a subset of the input sample predictions, the user must identify the dependence structure using the `compound_taskid_set` parameter to ensure the resulting ensemble is valid. The compound task ID set consists of task ID variables that do not display dependence amongst their values, and thus each unique combination of their values can be seen as representations of a single multivariate outcome of interest (or compound modeling task). If no task ID variables display dependents, then the `compound_taskid_set` consists of all task ID variables.

The example data for the sample output type has task ID variables "reference_date", "location", "horizon", "target", and "target_end_date"; of these five, only "horizon" displays dependence while "target_end_date" is derived from a combination of "reference_date" and "horizon". The other three task ID variables thus make up the compound task ID set, which is specified in the call to `linear_pool()`.
...
Derived task IDs generally are not part of the compound task ID set **unless** all the task ID variables their values depend upon are already part of the compound task ID set. The `derived_tasks` argument is used to validate the provided compound task ID set is compatible with the input sample predictions. If there are no derived tasks, then the argument may be set to `NULL`.
