...
hubExamples::forecast_outputs |>
  ...
  dplyr::select(model_id, target, horizon, output_type, output_type_id, value) |>
  knitr::kable(
    caption = "Example model output for forecasts of incident influenza
      hospitalizations and hospitalization intensity. A subset of is shown:
      1-week ahead quantile, median, and PMF forecasts made on 2022-12-17
      for Massachusetts from three distinct models. Only the 25th and 
      75th quantiles are displayed for the quantile forecasts. We round the 
      forecasts (the `value` column) to two digits. The `location`,
      `reference_date` and `target_end_date` columns are omitted for brevity. 
      This example data is provided in the `hubExamples` package."
  )

hubExamples::forecast_outputs
print(hubExamples::forecast_outputs)
head(hubExamples::forecast_outputs)
```

We also have corresponding target data included in the `hubExamples` package (\@ref(tab:example-target-data)). The example target data provide observed incident influenza hospitalizations (`observation`) in a given week (`date`) and for a given location (`location`). This target data could be used as calibration data for generating forecasts or for evaluating the forecasts post hoc. The forecast-specific task ID variables `reference_date` and `horizon` are not relevant for the target data.

```{r example-target-data, cols.print=13}
hubExamples::forecast_target_ts |>
  ...
  knitr::kable(
    caption = "Example target data for incident influenza hospitalizations. This
...
The resulting model output has the same structure as the original model output data (\@ref(tab:mean-ensemble)), with columns for model ID, task ID variables, output type, output type ID, and value. We also use `model_id = "simple-ensemble-mean"` to change the name of this ensemble in the resulting model output; if not specified, the default will be "hub-ensemble".

```{r mean-ensemble, echo=FALSE, rows.print = 9}
mean_ens |>
  dplyr::filter(
    output_type %in% c("quantile", "median", "pmf"),
    output_type_id %in% c(
  ...
  dplyr::select(model_id, target, horizon, output_type, output_type_id, value) |>
  knitr::kable(
    caption = "Mean ensemble model output. The values in the `model_id` column are
...
Like with `simple_ensemble()`, weights may be provided for each model that determine its contribution to the resulting ensemble.
...
We can also change the distribution that `distfromq` uses to estimate the component models' distributional tails to either log normal or cauchy using the `tail_dist` argument.
...
```{r plot-linear-pools, echo=FALSE, warning=FALSE, fig.height=4, fig.width=8, fig.cap = "Three different linear pools for weekly incident influenza hospitalizations in Massachusetts. Each linear pool combines individual predictions from the example hub using different distributional tails: normal, log normal, or cauchy. Linear pools are represented by a  median (line), 50% and 90% prediction intervals (ribbons). All generate  similar estimates, except for cauchy near the distribution tails."}
linear_pool_plot <- dplyr::bind_rows(
  linear_pool_norm, linear_pool_lnorm,
  linear_pool_cauchy
) |>
  dplyr::filter(
    location == "25",
...
hubVis::plot_step_ahead_model_output(
  model_output_data = linear_pool_plot,
  target_data = target_data_plot,
  use_median_as_point = TRUE,
  interactive = FALSE,
  intervals = c(0.5, 0.90),
...
The three resulting linear pools are mostly the same, except for the outer quantiles. This is expected given that the only difference is the selection of tail distribution. The linear pools with normal and lognormal tails are nearly identical while the linear pool with cauchy tails has a slightly wider 90% prediction intervals, which is as expected since cauchy distributions have heavy tails.
