...
The resulting model output has the same structure as the original model output data, with columns for model ID, task ID variables, output type, output type ID, and value. We also use `model_id = "simple-ensemble-mean"` to change the name of this ensemble in the resulting model output; if not specified, the default will be "hub-ensemble".

```{r mean-ensemble, echo=FALSE, rows.print = 9, max.width='300px'}
mean_ens |>
  dplyr::filter(
    output_type %in% c("quantile", "median", "pmf"),
    output_type_id %in% c(
...
The figure below compares three linear pools with different distribution tails—normal,lognormal, and cauchy—predicting weekly incident influence hospitalizations in Massachusetts. The plotted quantiles show that the three linear pools generate similar predictive distributions, except for the model with cauchy tails near the outer edges of the distribution. This is expected given that the only difference between the linear pools is the selection of tail distribution, and cauchy distributions have heavy tails.

```{r plot-linear-pools, echo=FALSE, warning=FALSE, fig.height=4, fig.width=8}
