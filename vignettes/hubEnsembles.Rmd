...
basic_plot_function <- function(plot_df, truth_df, plain_line = 0.5, ribbon = c(0.975, 0.025),
                                forecast_date) {
  plain_df <- dplyr::filter(plot_df, .data$output_type_id == plain_line)

  ribbon_df <- dplyr::filter(plot_df, .data$output_type_id %in% ribbon) %>%
    dplyr::mutate(output_type_id = ifelse(.data$output_type_id == min(ribbon),
      "min", "max"
    )) %>%
    tidyr::pivot_wider(names_from = .data$output_type_id, values_from = .data$value)
