...
v2_1 <- model_outputs$value[
  model_outputs$location == "222" &
    model_outputs$output_type_id == .1
] <-
  c(10, 30, 15, 20)
v2_5 <- model_outputs$value[
  model_outputs$location == "222" &
    model_outputs$output_type_id == .5
] <-
  c(40, 40, 45, 50)
v2_9 <- model_outputs$value[
  model_outputs$location == "222" &
    model_outputs$output_type_id == .9
] <-
  c(60, 70, 75, 80)
v8_1 <- model_outputs$value[
  model_outputs$location == "888" &
    model_outputs$output_type_id == .1
] <-
  c(100, 300, 400, 250)
v8_5 <- model_outputs$value[
  model_outputs$location == "888" &
    model_outputs$output_type_id == .5
] <-
  c(150, 325, 500, 300)
v8_9 <- model_outputs$value[
  model_outputs$location == "888" &
    model_outputs$output_type_id == .9
] <-
  c(250, 350, 500, 350)
...
test_that("component model outputs and resulting ensemble model outputs have identical sorted unique output type ids", {
  ...
  quantile_outputs$value[
    quantile_outputs$location == "222" &
...
test_that("group_by(output_type_id) produces expected results", {
  ...
  explicit_ensemble <- simple_ensemble(
    explicit_outputs,
    weights = NULL,
    weights_col_name = NULL,
    model_id = "hub-ensemble",
  ...
  implicit_ensemble <- simple_ensemble(
    implicit_outputs,
    weights = NULL,
    weights_col_name = NULL,
    model_id = "hub-ensemble",
...
test_that("(weighted) medians and means work with alternate name for weights columns", {
  weighted_median_actual <- model_outputs |>
    simple_ensemble(
      weights = fweight |> dplyr::rename(w = "weight"),
      weights_col_name = "w",
      agg_fun = "median"
    )
  ...
  weighted_median_expected <- simple_ensemble(
    model_out_tbl = model_outputs,
    weights = fweight,
    agg_fun = "median"
  )
...
test_that("passing agg_fun as symbol or character results in identical behaviour", {
  weighted_mean_actual_char <- simple_ensemble(
    model_out_tbl = model_outputs,
    weights = fweight,
    agg_fun = "mean"
  )
  weighted_mean_actual_symbol <- simple_ensemble(
    model_out_tbl = model_outputs,
    weights = fweight,
    agg_fun = mean
  )
