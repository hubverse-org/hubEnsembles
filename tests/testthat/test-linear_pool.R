test_that("(#128) linear pool will group by output_type", {
  ...
  expect_equal(
    res[res$output_type == "cdf", -1],
    ser[ser$output_type == "cdf", -1],
    tolerance = 1e-10
  )
})
...
test_that("group_by(output_type_id) produces expected results", {
  ...
  explicit_outputs$value[
    explicit_outputs$location == "222" &
  ...
  implicit_outputs$output_type_id <- sort(rep(
    c((1 - intervals) / 2, 1 - (1 - intervals) / 2),
    8
  ))

  explicit_ensemble <- linear_pool(
    explicit_outputs,
    weights = NULL,
    weights_col_name = NULL,
    model_id = "hub-ensemble",
...
test_that("The results are equivalent to those calculated by simple_ensemble for mean, cdf, and pmf output types", {
  ...
  weighted_mean_expected <- simple_ensemble(
    mean_outputs,
    weights = fweight1,
  ...
  weighted_mean_actual <- linear_pool(
    mean_outputs,
    weights = fweight1,
    weights_col_name = "weight",
    model_id = "hub-ensemble",
...
test_that("(weighted) quantiles correctly calculated", {
  ...
  output_prob <- stats::pnorm(quantile_values, mean = -3) /
    3 +
    stats::pnorm(quantile_values, mean = 0) / 3 +
    stats::pnorm(quantile_values, mean = 3) / 3
  weighted_output_prob <- 0.25 *
    stats::pnorm(quantile_values, mean = -3) +
  ...
  weighted_component_outputs$value[
    weighted_component_outputs$model_id == "a"
  ] <-
    stats::qnorm(weighted_output_prob, mean = -3)
  weighted_component_outputs$value[
    weighted_component_outputs$model_id == "b"
  ] <-
    stats::qnorm(weighted_output_prob, mean = 0)
  weighted_component_outputs$value[
    weighted_component_outputs$model_id == "c"
  ] <-
    stats::qnorm(weighted_output_prob, mean = 3)
  ...
  weighted_quantile_actual <- linear_pool(
    weighted_component_outputs,
    weights = fweight1,
    weights_col_name = "weight",
    model_id = "hub-ensemble",
    task_id_cols = NULL
...
test_that("(weighted) quantiles correctly calculated - lognormal family", {
  ...
  # The linear pool is a (weighted) mixture with cdf F(x) = \sum_m w_m F_m(x)
  # We test with equal weights w_m = 1/3 and with weights w_1 = 0.25, w_2 = 0.5, w_3 = 0.25
  quantile_values <- weighted_quantile_values <- exp(seq(
    from = -3,
    to = 3,
    by = 0.5
  )) # expected
  ...
  weighted_output_prob <- 0.25 *
    stats::plnorm(quantile_values, mean = -3) +
  ...
  quantile_actual_norm <- linear_pool(
    component_outputs,
    weights = NULL,
    weights_col_name = NULL,
    model_id = "hub-ensemble",
  ...
  weighted_quantile_actual_norm <- linear_pool(
    weighted_component_outputs,
    weights = fweight1,
    weights_col_name = "weight",
    model_id = "hub-ensemble",
  ...
  quantile_actual_lnorm <- linear_pool(
    component_outputs,
    weights = NULL,
    weights_col_name = NULL,
    model_id = "hub-ensemble",
  ...
  weighted_quantile_actual_lnorm <- linear_pool(
    weighted_component_outputs,
    weights = fweight1,
    weights_col_name = "weight",
    model_id = "hub-ensemble",
...
test_that("Not all component models forecasting for the same set of dependent task values throws an error", {
  ...
  sample_outputs_derived <- dplyr::mutate(
    sample_outputs,
    reference_date = target_date - 7 * horizon
  )
  sample_tasks_derived <- c(
    "reference_date",
    "location",
    "horizon",
    "target",
  ...
  missing_expected_derived <- create_test_sample_outputs() |>
    ...
    dplyr::distinct(dplyr::across(dplyr::all_of(c(
      "model_id",
      setdiff(sample_tasks_derived, "reference_date")
    ))))
...
test_that("If the specified `compound_taskid_set` is incompatible with component model outputs, throw an error", {
  ...
  sample_outputs <- create_test_sample_outputs() |>
    dplyr::mutate(
      output_type_id = paste0(.data[["location"]], .data[["output_type_id"]])
    )
...
test_that("remainder samples are properly distributed when component models don't all forecast for every location", {
  ...
  sample_outputs <- create_test_sample_outputs() |>
    dplyr::filter(
      (model_id %in% letters[1:3] | location == "222"),
      horizon == 1
    )
  ...
  num_model_forecasts_222 <- lp_outputs_summarized$num_forecasts_per_model[
    lp_outputs_summarized$location == "222"
  ]
  ...
  num_model_forecasts_888 <- lp_outputs_summarized$num_forecasts_per_model[
    lp_outputs_summarized$location == "888"
  ]
...
test_that("ensemble of samples correctly drawn for compound task ID sets", {
  ...
  subset_summarized_222 <- subset_summarized[
    subset_summarized$location == "222",
  ]
  ...
  subset_summarized_888 <- subset_summarized[
    subset_summarized$location == "888",
  ]
  ...
  all_tasks_summarized_222 <- all_tasks_summarized[
    all_tasks_summarized$location == "222",
  ]
  ...
  all_tasks_summarized_888 <- all_tasks_summarized[
    all_tasks_summarized$location == "888",
  ]
...
test_that("ensemble of samples throws an error for the more complex cases", {
  ...
  expect_error(
    ...
    "`weights` must be \"NULL\" or equal for every model",
    fixed = TRUE
  )
})
