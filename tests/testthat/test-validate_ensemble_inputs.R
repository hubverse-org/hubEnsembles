...
test_that("weights missing required columns generates error", {
  expect_error(
    ...
    "`weights` did not include required columns",
    fixed = TRUE
  )
})
...
test_that("value column in weights generates error", {
  expect_error(
    ...
    "`weights` included a column named \"value\", which is not allowed",
    fixed = TRUE
  )
})
...
test_that("weights column already in model_out_tbl generates error", {
  expect_error(
    model_outputs |>
      ...
      validate_ensemble_inputs(
        weights = fweight,
        valid_output_types = c("quantile")
      ),
    "is already a column in `model_out_tbl`",
    fixed = TRUE
  )
})
...
test_that("validated model outputs is a model_out_tbl", {
  validated_outputs <- validate_ensemble_inputs(
    model_outputs,
    valid_output_types = "quantile"
  )[[1]]
