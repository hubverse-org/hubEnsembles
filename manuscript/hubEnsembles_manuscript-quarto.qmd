...
author:
  ...
  - name: Emily Howerton
    ...
    affiliation: 
      - name: Department of Biology and Center for Infectious Disease Dynamics, Pennsylvania State University, University Park
        city: State College
        state: PA
...
license: "CC BY"
copyright: 
  holder: Consortium of Infectious Disease Modeling Hubs
  year: 2024
citation: 
...
The probability average or linear pool is calculated by averaging probabilities across predictions for a fixed value of the target variable, $x$. This can be expressed in terms of either predictive CDFs or PDFs as follows: \begin{align*}
...
: A table summarizing how the model output representation columns are used for predictions of different output types. Adapted from [@hubverse_docs]. {#tbl-model-out}
...
p1 <- dplyr::bind_rows(
  ...
  labs(
    x = "1-week change in US incident hospitalizations",
    y = "probability"
  ) +
