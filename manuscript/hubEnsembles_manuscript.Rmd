...
To understand how these methods are implemented in `hubEnsembles`, we first need to understand the common forecasting conventions that have been established and used by the hubverse. Each package within the larger suite of tools follows the same set of outlined rules. We begin with a short overview of concepts and conventions needed to understand and utilize the `hubEnsembles` package, then explain the implementation of each ensembling function.
...
The model output representation specifies how the forecasts are being conveyed and consists of three columns: (1) output_type, (2) output_type_id, and (3) value. The output_type defines how the predictive distribution is represented and may be one of mean or median (point forecasts), quantile, cdf, or pmf (distributional forecasts). The output_type_id provides more identifying information for a forecast and is specific to the particular output_type.  The value contains the actual numerical prediction. Additionally, unlike for the task IDs, these three columns are required and their names are fixed [@hubverse_docs].
...
EH: I think this is a great idea! It seems like it would be useful to cite at the end of the second to last sentence of the above paragraph. -->
...
We score the ensemble forecasts for every unique combination of task ID variables using three common metrics in forecast evaluation, including weighted interval score (WIS) [@bracher_evaluating_2021], MAE, 50% PI coverage, and 95% PI coverage. The scores are then grouped together and averages are taken of these metrics over different combinations of five variables: none, season, horizon, forecast date, and location. We display a table of the overall results (no grouping) and figures showing average WIS, MAE, and 95% PI coverage plotted against forecast date, horizon, and season.
