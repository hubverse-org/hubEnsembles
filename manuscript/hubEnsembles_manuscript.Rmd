...
vt_plot <- plot_forecasts(forecasts_vt, 
                    hub = "FluSight",
                    intervals = c(.50, .95),
...
ca_plot <- plot_forecasts(forecasts_ca, 
                    hub = "FluSight",
                    intervals = c(.50, .95),
                    truth_data = select(flu_truth_all, model, location, target_end_date, target_variable, value),
                    truth_source = "HealthData",
...
max_truth_ca <- max(filter(flu_truth_all, location=="48")$value)
max_forecast_ca <- max(filter(forecasts_ca, location=="48")$value)
