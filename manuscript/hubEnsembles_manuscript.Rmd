...
pA <- dplyr::bind_rows(
    ...
    ggplot(aes(x = q, y = p, color = dist)) + 
    geom_line(data = dplyr::bind_rows(
        data.frame(x = x, y = cdf_normal1, dist = "N(100, 10)"),
        data.frame(x = x, y = cdf_normal2,dist = "N(120, 5)"),
    ),
    mapping = aes(x = x, y = y),
    linewidth = 0.8, alpha = 0.8) +
    geom_segment(aes(x = q, xend = q, 
                     y = 0, yend = p, 
                     color = dist), 
    ...
    labs(x = "value", y = "cumulative probability (quantile)", 
         subtitle = "quantile predictions from two models") +
    scale_color_viridis_d(
        "Distribution",
        end = 0.9
      )  +
...
example_LOP_lines <- seq(85, 135, 10)
example_LOP_points <- data.frame(value = rep(example_LOP_lines,2), 
                                 quantile = c(pnorm(example_LOP_lines, mean = mean1, sd = sd1), 
                                              pnorm(example_LOP_lines, mean = mean2, sd = sd2)), 
                                 dist = c(rep("A", length(example_LOP_lines)), 
                                          rep("B", length(example_LOP_lines))))

example_LOP <- data.frame(value = x, 
                          quantile = rowMeans(cbind(cdf_normal1, cdf_normal2))
                                              )

example_quant_lines <- quantile_probs
example_quant_points <- data.frame(quantile = rep(example_quant_lines,2), 
                                 value = c(qnorm(example_quant_lines, mean = mean1, sd = sd1), 
                                              qnorm(example_quant_lines, mean = mean2, sd = sd2)), 
                                 dist = c(rep("A", length(example_quant_lines)), 
                                          rep("B", length(example_quant_lines))))
example_quant <- data.frame(value = x, 
                            quantile = pnorm(x, mean = mean(c(mean1, mean2)),
                                             sd = mean(c(sd1,sd2)))
                                              )

pB <- dplyr::bind_rows(
    data.frame(x = x, y = cdf_normal1, dist = "A"),
    data.frame(x = x, y = cdf_normal2,dist = "B")) |>
    ggplot() +
    geom_segment(
        data = example_quant_points |>
            reshape2::dcast(quantile ~ dist, value.var = "value"), 
    ...
    geom_point(
        data = example_quant_points,
        mapping = aes(x = value, y = quantile, color = dist),
        size = 1.2
    ) +
    geom_line(
        data = example_quant, 
...
pB_inset <- ggplot(data = pdfs |> 
                       filter(distribution != "LOP"),
                   mapping = aes(x = value, y = prob, 
                                 color = distribution, 
                                 linewidth = distribution)) + 
    geom_line() + 
    coord_cartesian(xlim=c(75, 135)) +
    scale_color_manual(values = c(viridis::viridis(6)[c(3,6)], "black")) +
...
pC <- dplyr::bind_rows(
    ...
    ggplot() +
    geom_segment(
        data = example_LOP_points |>
            reshape2::dcast(value ~ dist, value.var = "quantile"), 
        mapping = aes(x = value, xend = value, 
                      y = A, yend = B), 
        color = "darkgray", linetype = "dashed") +
    # geom_vline(
    #     xintercept = example_LOP_lines, 
    ...
    geom_point(
        data = example_LOP_points |>
            group_by(value) |>
            summarize(quantile = mean(quantile)), 
        mapping = aes(x = value, y = quantile),
        size = 1.2
    ) + 
    labs(x = "value", y = "cumulative probability (quantile)", 
         subtitle = "linear pool average") +
    scale_color_viridis_d(
        "Distribution",
        end = 0.9, 
        labels = c("N(100, 10)"), "N(120, 5)"
...
pC_inset <- ggplot(data = pdfs |> 
                       filter(distribution != "Q"),
                   mapping = aes(x = value, y = prob, 
                                 color = distribution, 
                                 linewidth = distribution)) + 
    geom_line() + 
...
plot_grid(
    pA, 
    plot_grid(pC, pB, pC_inset, pB_inset, nrow = 2)
)
ggsave("test2.pdf", width = 8, height = 4)
...
plot_grid(
    ...
    l, 
    ncol = 1, rel_heights = c(0.95, 0.05))
ggsave("test3.pdf", width = 10, height = 4)
...
As expected, the mean, median, and geometric mean each give us slightly different resulting ensembles. The median point estimates in Figure \@ref(fig:plot-ensembles) demonstrate this. 

```{r plot-ensembles, fig.cap = 'Plot of three different ensembles (arithmetic mean, geometric mean, and median) for daily COVID-19 incident hospitalizations combining three existing models: simple_hub-baseline (black), UMass-ar (dark gray), and UMass-gbq (light gray). Truth data is plotted in black with daily values shown as points.'}
