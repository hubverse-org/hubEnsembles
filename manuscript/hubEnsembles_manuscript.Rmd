...
We will use the `example-simple-forecast-hub`, which has been created by the hubverse team as a simple example hub to demonstrate the set up and functionality for the hubverse (https://github.com/Infectious-Disease-Modeling-Hubs/example-simple-forecast-hub). The hub includes both example model output data and target data (sometimes known as "truth" data). The model output data includes `quantile`, `mean` and `median` forecasts of future incident COVID-19 hospitalizations, and `pmf` forecasts of the probability that the change in hospitalizations will be a "large decrease", "decrease", "stable", "increase", or "large increase". 
...
```{r}
target_data |>
  dplyr::filter(location == "US", time_idx >= "2022-12-12") |>
  head()
```
...
```{r plot-ex-mods, fig.width = 8, fig.height = 4, fig.cap = "Example model output. One example forecast of daily US incident COVID-19 hospitalizations is shown for each of three models (panels). Forecasts are represented by a median (line) and 50% prediction interval (Q25-Q75)."}
model_outputs_plot <- model_outputs |>
...
geometric_mean_ens <-
  hubEnsembles::simple_ensemble(model_outputs, agg_fun = geometric_mean,
...
We can weight the contributions of each model in the ensemble by providing a `data.frame` that specifies these weights for the `weights` argument of `simple_ensemble`. This `data.frame` should include a `model_id` column containing each unique model_id and a `weight` column. In the following example, we include the baseline model in the ensemble, but give it less weight than the other forecasts.

```{r}
model_weights <- data.frame(model_id = c("UMass-ar", "UMass-gbq",
                                         "simple_hub-baseline"),
                            weight = c(0.4, 0.4, 0.2))

weighted_mean_ens <-
  hubEnsembles::simple_ensemble(model_outputs, weights = model_weights,
...
```{r}
linear_pool_ens <-
  hubEnsembles::linear_pool(dplyr::filter(model_outputs,
                                          output_type != "median"),
...
p1 <- dplyr::bind_rows(mean_ens,
                       linear_pool_ens) |>
  dplyr::filter(output_type == "pmf", origin_date == "2022-12-12", horizon == 7,
                location == "US") |>
  dplyr::mutate(output_type_id = gsub("_", " ", output_type_id)) |>
  dplyr::mutate(output_type_id = factor(output_type_id,
                                        levels = c("large decrease", "decrease",
                                                   "stable", "increase",
                                                   "large increase"))) |>
  ggplot(aes(x = output_type_id, y = value, fill = model_id)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "1-week change in US incident hospitalizations", y = "probability") +
