...
By default, `simple_ensemble` uses the mean for the aggregation function $C$ and equal weights for all models. For point predictions with a mean or median output type, the resulting ensemble prediction is an equally weighted average of the individual models' predictions. For probabilistic predictions in a quantile format, by default `simple_ensemble` produces a quantile average, and for model outputs in a CDF or PMF format, by default `simple_ensemble` computes an equally weighted linear opinion pool.
...
For step 1, functionality in the `distfromq` package uses a monotonic cubic spline for interpolation on the interior of the provided quantiles. The user may choose one of several distributions to perform extrapolation of the CDF tails. These include normal, lognormal, and cauchy distributions, with "normal" set as the default. A location-scale parameterization is used, with separate location and scale parameters chosen in the lower and upper tails so as to match the two most extreme quantiles [@distfromq].
...
The target data provides the observed value (`value`) for the forecast target (`target`) on a given single day (`time_idx`) and in a given location (`location`).
```{r}
head(target_data |> 
...
We can plot these forecasts and the target data using the `plot_step_ahead_model_output()` function from `hubVis`, another hubverse package that visualizes model outputs.

```{r plot-ex-mods, fig.width = 8, fig.height = 4, fig.cap = "Sample model output from the example hub. One example forecast of daily US incident COVID-19 hospitalizations is shown for each three models (panels). Forecasts are represented by a median (line) and 50% prediction interval (Q25-Q75)."}
...
The resulting model output has the same structure as the original model output data, with columns for the task ID variables, output type, output type ID, and the model ID.
```{r}
head(mean_ens |>
         dplyr::filter(output_type %in% c("quantile","median"), 
                       output_type_id %in% c(0.01, 0.1, 0.25, 0.75, 0.9, 0.99, NA),
...
As expected, the mean, median, and geometric mean each give us slightly different resulting ensembles. The median point estimates and 50% prediction intervals (i.e., 25th and 75th quantiles) in Figure \@ref(fig:plot-ensembles) demonstrate this. 

```{r plot-ensembles, fig.height = 4, fig.width = 6, fig.cap = 'Three different ensembles for daily US incident COVID-19 hospitalizations. Ensembles combine individual models from the example hub (Fig \\@ref(fig:plot-ex-mods)) using three methods: arithmetic mean (green), geometric mean (purple), and median(orange). All methods correspond to variations of the quantile average approach.'}
...
We can also generate a linear pool ensemble, or distributional mixture, using the `linear_pool()` function; this function can only be applied to predictions with an `output_type` of `mean`, `quantile`, `cdf`, or `pmf`. Our example hub includes `median` output type, so we exclude it from the calculation; however we can add it back using Q50.  
...
```{r plot-ex-quantile-and-linear-pool, fig.width = 6, fig.height = 4, fig.cap = "Comparison of quantile average (red) and linear pool (blue) ensembles for daily US incident COVID-19 hospitalizations. Ensmebles combine individual models from the example hub (Fig \\@ref(fig:plot-ex-mods))."}
hubVis::plot_step_ahead_model_output(model_output_data = dplyr::bind_rows(mean_ens,
                                                                          linear_pool_ens |>
