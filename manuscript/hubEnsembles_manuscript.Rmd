...
The resulting model output has the same structure as the original model output data, with columns for the model ID, task ID variables, output type, output type ID, and value.
```{r}
head(mean_ens |>
         dplyr::filter(output_type %in% c("quantile","median"), 
                       output_type_id %in% c(0.01, 0.1, 0.25, 0.75, 0.9, 0.99, NA),
