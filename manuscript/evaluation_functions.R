...
#'  row per model and horizon combination plus a `horizon` column
...
#'  specified summary metric vs horizon
#' @export
#'
#' @examples
plot_evaluated_scores <- function(summarized_scores, model_names, model_colors,
                                  y_var = "wis", main) {

  if (y_var == "wis") {
    gg <- ggplot2::ggplot(summarized_scores,
                          mapping = ggplot2::aes(x = horizon, y = wis,
                                                 group = model)) +
      ggplot2::coord_cartesian(
        ylim = c(min(summarized_scores$wis) * 0.9,
                 median(dplyr::filter(
                   summarized_scores, horizon == 4)$wis) * 1.5))
  } else if (y_var == "mae") {
    gg <- ggplot2::ggplot(summarized_scores,
                          mapping = ggplot2::aes(x = horizon, y = mae,
                                                 group = model)) +
      ggplot2::coord_cartesian(
        ylim = c(min(summarized_scores$mae) * 0.9,
                 median(dplyr::filter(summarized_scores, horizon == 4)$mae) *
                   1.5))
  } else if (y_var == "cov95") {
    gg <- ggplot2::ggplot(summarized_scores,
                          mapping = ggplot2::aes(x = horizon, y = cov95,
                                                 group = model)) +
  ...
  gg +
    ...
    ggplot2::labs(title = main, x = "horizon", y = paste("average", y_var)) +
    ggplot2::theme_bw()
}
...
pairwise_comparison <-
  function(heat_scores, mx, my, subset = rep(TRUE, nrow(heat_scores)),
  ...
  if (permutation_test) {
    ...
    sub_fcd <- stats::aggregate(cbind(wis.x, wis.y) ~ forecast_date,
                                data = sub, FUN = mean)
  ...
  return(list(ratio = ratio, pval = pval, pval_fcd = pval_fcd, mx = mx,
              my = my))
}
...
#' @param baseline_name Name of baseline model (must be included in
#'  `model_levels`)
#'
#' @return A tile plot of pairwise relative WIS broken down by location and
#'  model using the scores provided
#' @export
#'
#' @examples
plot_wis_loc <- function(scores, truth, model_levels, baseline_name) {
  # potentially add choice of wis, mae, coverage?

  # reorder states, reorder models
  data("hub_locations")

  inc_scores <- scores %>%
    dplyr::filter(!(location %in% c("22", "US") &
...
#  inc_scores$forecast_date <- next_monday(inc_scores$forecast_date)
  ...
  heat_scores <- inc_scores %>%
    dplyr::left_join(covidHubUtils::hub_locations_flusight,
                     by = c("location" = "fips")) %>%
    dplyr::select("model", "forecast_date", "location", "location_name",
                  "horizon", "abs_error", "wis", "horizon") %>%
    droplevels()
  ...
  for (i in seq_along(locations)) {
    ...
    for (mx in seq_along(models)) {
      for (my in 1:mx) {
        pwc <- pairwise_comparison(
          heat_scores = dplyr::filter(heat_scores, horizon %in% 1:4),
          mx = models[mx], my = models[my],
          permutation_test = FALSE,
          # disable permutation test to speed up things
          subset = dplyr::filter(heat_scores, horizon %in% 1:4)$location == loc
    ...
    geom_mean_ratios_temp <- exp(rowMeans(log(
      results_ratio_temp[, -ind_baseline]), na.rm = TRUE))
    ratios_baseline_temp <- results_ratio_temp[, baseline_name]
    ratios_baseline2_temp <- geom_mean_ratios_temp /
      geom_mean_ratios_temp[baseline_name]
  ...
  truth_dat <- truth %>%
    ...
    dplyr::ungroup() %>%
    dplyr::mutate(location_name = stats::reorder(location_name, cum_value)) %>%
    dplyr::pull(location_name)

  average_by_loc_to_plot <- average_by_loc %>%
    dplyr::mutate(location_name = fct_relevel(location_name, levels(truth_dat)),
  ...
  fig_wis_loc <- average_by_loc_to_plot %>%
    ggplot2::ggplot(ggplot2::aes(x = model, y = location_name,
                                 fill = scales::oob_squish(
                                   log_relative_wis, range = c(-2, 1.5)))) +
    ggplot2::geom_tile() +
